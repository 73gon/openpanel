...
export const Route = createFileRoute('/series/$seriesId')({
  loader: async ({ params }) => {
    ...
    const [metadata, progressMap, chaptersData, continueData] = await Promise.all([
...
function SeriesDetailPage() {
  ...
  const [continueInfo, setContinueInfo] = useState<SeriesContinueResponse | null>(
    loaderData.continueData,
  )
  ...
  const handleRescan = async () => {
    ...
    try {
      ...
      const freshContinue = await fetchSeriesContinue(seriesId).catch(() => null)
      setContinueInfo(freshContinue)
                ...
                >
                  <div className="group relative overflow-hidden rounded-md bg-foreground text-background transition-all hover:opacity-90">
                    ...
                    <div className="relative flex items-center gap-1.5 px-3 py-1.5">
                      <HugeiconsIcon icon={PlayIcon} size={12} strokeWidth={3} className="shrink-0" />
          ...
          {displayMode === 'chapters' && seriesChapters.length > 0 ? (
                      ...
                      <div className="group relative cursor-pointer overflow-hidden rounded-lg border border-border/50 bg-card px-4 py-3 transition-all hover:border-border hover:bg-accent/50">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-medium">{stripTitleZeros(ch.title)}</p>
                              <p className="text-xs text-muted-foreground">
                                {stripTitleZeros(ch.book_title)} · Pages {ch.start_page + 1}–
                                {ch.end_page + 1}
                              </p>
                            </div>
                          </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              {books.map((book, i) => {
                ...
                const cardContent = (
                      <div className={`group relative cursor-pointer overflow-hidden rounded-lg transition-all hover:shadow-md ${selectMode && isSelected ? 'ring-2 ring-primary' : ''}`}>
                        {selectMode && (
                          <div className="absolute left-1.5 top-1.5 z-20">
                            <div className={`flex h-5 w-5 items-center justify-center rounded border-2 transition-colors ${isSelected ? 'border-primary bg-primary text-white' : 'border-white/80 bg-black/40'}`}>
                          ...
                          {isPWA && (
                            ...
                            >
                              ...
                              downloadStatuses[book.id]?.status ===
                                ...
                                <button className="flex h-7 w-7 items-center justify-center rounded-full bg-black/60 text-white backdrop-blur-sm transition-all active:scale-90" aria-label="Download">
                                  <HugeiconsIcon
                                    icon={Download04Icon}
                                    size={14}
                                  />
                ...
                return (
                  <motion.div
                    ...
                    onClick={selectMode ? () => toggleSelectBook(book.id) : undefined}
                  >
                    {selectMode ? cardContent : (
            ...
            <div className="grid gap-2">
              {books.map((book, i) => {
                ...
                const listContent = (
                      <div className={`group relative cursor-pointer overflow-hidden rounded-lg border border-border/50 bg-card px-4 py-3 transition-all hover:border-border hover:bg-accent/50 ${selectMode && isSelected ? 'ring-2 ring-primary' : ''}`}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            {selectMode && (
                              <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded border-2 transition-colors ${isSelected ? 'border-primary bg-primary text-white' : 'border-muted-foreground/40'}`}>
                                {isSelected && <HugeiconsIcon icon={Tick01Icon} size={12} />}
                              </div>
                            )}
                            ...
                            {isPWA && (
                              ...
                              >
                                ...
                                downloadStatuses[book.id]?.status ===
                                  ...
                                  <button className="flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground transition-all active:scale-90" aria-label="Download">
                                    <HugeiconsIcon
                                      icon={Download04Icon}
                                      size={14}
                                    />
