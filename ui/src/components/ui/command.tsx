...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent
        className={cn(
          'top-1/3 translate-y-0 overflow-hidden rounded-none p-0 sm:max-w-lg lg:max-w-xl',
          className,
        )}
      ...
      >
        <Command
          filter={filter}
          className="**:[[cmdk-group-heading]]:text-muted-foreground **:[[cmdk-group-heading]]:px-2 **:[[cmdk-group-heading]]:text-xs"
        >
          {children}
        </Command>
      </DialogContent>
    ...
    <div data-slot="command-input-wrapper" className="border-b pb-0">
      <InputGroup className="bg-input/30 border-input/30 h-8 border-none shadow-none! *:data-[slot=input-group-addon]:pl-2!">
        <CommandPrimitive.Input
          ...
          autoFocus
          className={cn(
            'w-full text-base md:text-xs outline-hidden disabled:cursor-not-allowed disabled:opacity-50',
            className,
          )}
          {...props}
        />
